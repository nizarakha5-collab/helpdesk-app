...
<div class="dashboard">
    <aside class="sidebar">
        ...
        <nav class="menu">
            ...
            <a href="{{ route('agent.chat.index') }}" class="menu-item active">
                <span class="menu-icon">💬</span>
                <span>Chat admin</span>
                <span class="badge">{{ $adminChatUnreadCount }}</span>
            </a>

            <a href="{{ route('agent.history') }}" class="menu-item">
                <span class="menu-icon">🕘</span>
                <span>Historique</span>
            </a>

            <a href="{{ route('agent.reports') }}" class="menu-item">
                <span class="menu-icon">📊</span>
                <span>Rapports</span>
            </a>
        </nav>
    ...
    <main class="main">
        <header class="topbar">
            <div class="topbar-title">Chat admin</div>
            ...
            <div class="chat-shell">
                <aside class="contacts-panel">
                    <div class="contacts-head">
                        <div class="contacts-title">Administrateurs</div>
                        <div class="contacts-text">
                            Choisissez un administrateur pour démarrer ou continuer la conversation.
                        </div>
                    </div>
                    ...
                    <div class="contacts-list">
                        @forelse ($contacts as $item)
                            @php
                                ...
                                $isActive = $selectedAdmin && $selectedAdmin->id === $contact->id;
                            @endphp

                            <a href="{{ route('agent.chat.index', ['adminUser' => $contact->id]) }}" class="contact-item {{ $isActive ? 'active' : '' }}">
                                ...
                                <div class="contact-body">
                                    ...
                                    <div class="contact-preview">
                                        <div class="contact-preview-text">
                                            @if ($lastMessage)
                                                @if ($lastMessage->sender_id === $agent->id)
                                                    Vous:
                                                @endif
                        ...
                        @empty
                            <div class="empty-contact">
                                Aucun administrateur actif disponible.
                            </div>
                        @endforelse
                    </div>
                </aside>
                ...
                <section class="conversation-panel">
                    @if ($selectedAdmin)
                        <div class="conversation-head">
                            <div class="conversation-user">
                                <div class="conversation-avatar">{{ strtoupper(substr($selectedAdmin->username, 0, 1)) }}</div>

                                <div style="min-width:0;">
                                    <div class="conversation-name">{{ $selectedAdmin->username }}</div>
                                    <div class="conversation-sub">{{ $selectedAdmin->email }} · Administrateur</div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="conversation-body" id="conversationBody">
                                ...
                                <div class="message-row {{ $isMine ? 'mine' : 'other' }}">
                                    <div class="message-bubble {{ $isMine ? 'mine' : 'other' }}">
                                        <div class="message-meta">
                                            {{ $isMine ? 'Vous' : ($message->sender?->username ?? 'Admin') }}
                                            ·
                            ...
                            @empty
                                <div class="conversation-empty">
                                    Aucun message pour le moment. Commencez la conversation avec cet administrateur.
                                </div>
                            @endforelse
                        </div>

                        <div class="composer">
                            <form method="POST" action="{{ route('agent.chat.send', ['adminUser' => $selectedAdmin->id]) }}" class="composer-form">
                                @csrf
                    ...
                    @else
                        <div class="conversation-empty">
                            Sélectionnez un administrateur dans la liste de gauche pour afficher la conversation.
                        </div>
                    @endif
                </section>
            </div>
